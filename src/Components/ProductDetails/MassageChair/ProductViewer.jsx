...
const ProductViewer = ({ chair }) => {
  ...
  const mc1 = chair && chair?.images?.showcase[0];
  const mc2 = chair && chair?.images?.showcase[1];
  const mc3 = chair && chair?.images?.showcase[2];
  const mc4 = chair && chair?.images?.showcase[3];
  ...
  const onSubmit = (data) => {
    ...
    fetch(
      `https://emotion-products-server-new.vercel.app/api/v1/appointments`,
      {
        method: "POST",
        headers: {
    ...
    <div className="p-5">
      ...
      <div className="w-full flex flex-col md:flex-row gap-5">
        ...
        <div className="w-full md:w-2/5 space-y-3">
          ...
          {chair?.cmin && <div className="py-3">
            <h3 className="text-sm">
              {" "}
              <span className="font-bold">CM</span> | INCH{" "}
            </h3>
            <h5 className="text-xs font-semibold">{chair?.cminDesc}</h5>
          </div>}
          <div className="space-y-5 mb-5">
              ...
              <div className="py-7 w-full">
                ...
                <div className="w-full text-left pt-5 flex flex-col md:flex-row justify-start items-end gap-5">
                  ...
                  <div className="flex flex-col gap-3 items-end">
                    <button
                      onClick={() => navigate(`/massagechair/cart/${chair?.id}`)}
            ...
            <div className="space-y-5 bg-gray-100 rounded p-5 text-xs pt-5">
              <div className="flex items-center gap-3 tracking-widest">
                <FaCheck className="text-primary" />{chair?.checkers[0]}
              </div>
              <div className="flex items-center gap-3 tracking-widest">
