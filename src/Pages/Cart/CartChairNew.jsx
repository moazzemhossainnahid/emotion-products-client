...
const CartChairNew = () => {
  ...
  const [totalPrice, setTotalPrice] = useState(chair?.price);
  const [downPayment, setDownPayment] = useState(0);
  const [error, setError] = useState("");
  const downPayRef = useRef(null);

  useEffect(() => {
    const newPrice = chair?.price * quantity || 0;
    setTotalPrice(downPayment > 0 ? downPayment : newPrice);
  }, [chair, downPayment, quantity]);
  ...
  const processDownPayment = () => {
    ...
    const expectedDownPayment = (chair?.price * quantity) / 2; // 50% of total price
                ...
                <div>
                  ...
                  <div className="flex items-center font-semibold justify-between pt-7">
                    ...
                    <p className="text-xl text-gray-800 flex gap-2 items-center">
                      <FaEuroSign /> {(chair?.price * quantity)?.toFixed(2)}
                    </p>
                  </div>
                ...
                <div>
                  ...
                  <div
                    className={`${
                      error ||
                      (downPayment > 0 &&
                        downPayment < (chair?.price * quantity) / 2)
